...
export function usePortfolioFilter<T extends FilterableItem>(items: T[]) {
  ...
  const sortorder = ref(route.query.sortorder ? String(route.query.sortorder) : 'desc')
  const selectedRole = ref(route.query.role ? String(route.query.role) : '')
  const selectedPlatform = ref(route.query.platform ? String(route.query.platform) : '')
  ...
  const filteredList = computed(() =>
    items
      .filter((p) => {
        const matchesRole =
          selectedRole.value === '' || p.roles[locale.value].includes(selectedRole.value)
        const matchesPlatform =
          selectedPlatform.value === '' || p.platform.includes(selectedPlatform.value)
